...
export default class CustomerController {
  async login(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (customer) {
        const validPassword = await authHandler.validatePassword(
          password,
          customer.password,
          customer.salt
        );
        if (validPassword) {
          const token = await authHandler.generateToken({
  ...
  async createAddress(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const savedAddress = await customerRepository.addAddress(
        "65a37ca7fe8f1036394ee850",
        address
      );
  ...
  addToCart = async (data: any) => {
    await customerRepository.handleCart(data);
  };

  placeOrder = async (data: any) => {
    await customerRepository.createOrder(data);
  };
